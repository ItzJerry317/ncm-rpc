...
function getNCMPid() {
  ...
  if (namelist.length === 0) {
    console.log(chalk.red("[Function getNCMPid()] 未检测到网易云进程。请检查网易云是否打开或网易云进程名是否为cloudmusic.exe"))
    return undefined
  } else {
    ...
    if (ncmpid === undefined) {
      console.log(chalk.red("[Function getNCMPid()] 已检测到网易云进程，但未检测到网易云主窗口进程，请将网易云主窗口打开"))
      return undefined
    }
  }
}

function getSong(ncmpid) {
  //进程标题获取
  if (ncmpid != undefined) {
    var ncmTitle = hmc.getProcessHandle(ncmpid)
    var ncmSong = ncmTitle.title
    return ncmSong
  }
}

module.exports = { getNCMPid, getSong }